# 🔒 Security Policy — Jirnyak/macromac
...
> Project Scope: `MacroMac macOS HID Macro Automation & Event Injection Engine`
...
We actively maintain and provide critical security updates for the following release lines of **Jirnyak/macromac**:
...
Security engineering in Jirnyak/macromac is guided by the following domain-specific threat vector analyses:

### 1. Privilege Escalation via Macro Hooks
* **Description**: Intercepting or forging elevated system events to run unauthenticated root commands.
...
### 2. Permanent Modifier Key Lock
* **Description**: Application crash leaving Command, Control, or Option flags in permanent pressed state.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
### 3. Memory Corruption in JSON Parser
* **Description**: Malformed repeat block payloads overflowing C++ fixed-size vector buffers.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
...
If you discover a security flaw or exploit vector in **Jirnyak/macromac**, do **NOT** post it publicly in open issues or discussions.
